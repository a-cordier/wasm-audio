...
import type { ControlState, KnobSkin } from "./types";

const ANGLE_MIN = -135;
const ANGLE_MAX = 135;

const HANDLE_PATH = `M 249.52539,5.6313593e-5 A 250,250 0 0 0 
...
const CURSOR_PATH = `M 249.37207,1.108327e-4 A 250,273.78195 0 0 0 
      244.34472,0.06636606 V 53.60947 h 11.31055 V 0.07497377 a 
      250,273.78195 0 0 0 -5.80859,-0.07490674242 250,273.78195 
      0 0 0 -0.47461,0 z`;

export class KnobDefaultSkin implements KnobSkin {
  render(state: ControlState) {
    const angle = ANGLE_MIN + state.value * (ANGLE_MAX - ANGLE_MIN);
    ...
    return html`
      ...
      >
        ...
        <g transform="rotate(${angle}, 250, 250)">
          <path
            d="${HANDLE_PATH}"
            style="fill: var(--control-handle-color, #ccc)"
          />

          <path
            d="${CURSOR_PATH}"
            style="fill: var(--control-cursor-color, #ccc)"
          />

          <circle
            r="150" cy="250" cx="250"
            style="fill: var(--control-top-color, #ccc)"
          />
        </g>
      </svg>
    `;
...
export const knobDefaultSkin = new KnobDefaultSkin();
